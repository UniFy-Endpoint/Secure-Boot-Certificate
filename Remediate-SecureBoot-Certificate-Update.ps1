...
.TAGS           Intune SecureBoot UEFI Certificate Remediation ProactiveRemediation
.RELEASENOTES
    1.0.0 - Initial release. Remediation-only script for Intune Proactive Remediations.
#>

<#
.SYNOPSIS
    Remediation script for Secure Boot Windows UEFI CA 2023 compliance.
    Designed for use as the Remediation half of an Intune Proactive Remediation pair.

.DESCRIPTION
    Triggers the Windows built-in Secure Boot CA 2023 certificate update mechanism by
    setting the required registry trigger and starting the Windows scheduled task.

    Use this script alongside Detect-SecureBoot-Certificate-Update.ps1 in an Intune
    Proactive Remediation. Intune runs this script automatically when the detection
    script exits 1 (non-compliant).

    Exit codes (Intune Proactive Remediation convention):
      0 = Success — Remediation steps were applied, or the device was already compliant.
                    Intune marks the device as remediated.
      1 = Failure — Remediation could not be applied (incompatible device, script error,
                    or required conditions not met). Intune will retry on the next cycle.

    Remediation steps (only when non-compliant and device is compatible):
      1. Verifies device is UEFI, Secure Boot enabled, and not a virtual machine.
      2. If Intune Settings Catalog is managing Secure Boot updates (MDM guard):
           - Skips AvailableUpdates registry write to avoid MDM conflict.
           - Only starts the scheduled task as a safe complementary action.
      3. If no MDM conflict:
           - Creates the registry key path if missing.
           - Sets HKLM:\...\SecureBoot\AvailableUpdates = 0x5944 (idempotent).
           - Starts the Windows scheduled task \Microsoft\Windows\PI\Secure-Boot-Update.
    ...
    Safety guarantees:
      ...
      - Idempotent: safe to run multiple times (registry write skipped if value unchanged).
      - Skips remediation on BIOS/Legacy, Secure Boot disabled, or virtual machines.
...
.EXAMPLE
    ...
    Applies remediation. Exit 0 = success, exit 1 = failure.
...
.NOTES
    ...
    Part of the Intune Proactive Remediation pair:
      Detection  : Detect-SecureBoot-Certificate-Update.ps1
      Remediation: Remediate-SecureBoot-Certificate-Update.ps1  (this file)
...
$ScriptName    = 'Remediate-SecureBoot-Certificate-Update'
$ScriptVersion = '1.0.0'
...
# Windows Application event log source name for remediation results.
...
#
# When MicrosoftUpdateManagedOptIn = 1, the Settings Catalog policy set
# is active. The script will NOT write AvailableUpdates in this case,
# to avoid conflicting with MDM-managed values, and will only start the
# scheduled task as a safe complementary action.
# -----------------------------------------------------------------------
...
function Invoke-Detection {
    ...
    .DESCRIPTION
        Used by the remediation script to short-circuit if the device is already compliant
        (e.g. Windows Update applied the certificate since the detection script last ran).
        Returns $true = compliant (skip remediation), $false = non-compliant (remediate).
    .OUTPUTS
    ...
    if ($Diagnostics['SecureBootDBHas2023'] -eq $true) {
        Write-Host 'Already compliant: Windows UEFI CA 2023 certificate found in Secure Boot DB (binary-verified). Remediation not needed.'
        return $true
    }

    # EARLY EXIT: fundamental prerequisites
    if ($Diagnostics['SecureBootEnabled'] -notmatch 'Enabled') {
        Write-Warning "Cannot remediate: Secure Boot is '$($Diagnostics['SecureBootEnabled'])'. Enable Secure Boot in firmware settings first."
        return $false
    }
    if ($Diagnostics['FirmwareType'] -ne 'UEFI') {
        Write-Warning "Cannot remediate: Legacy BIOS/non-UEFI firmware detected. Secure Boot CA 2023 requires UEFI firmware."
        return $false
    }
    ...
    switch ($status) {
        'Updated' {
            if ($capable -eq '0x2' -and $noError) {
                Write-Host "Already compliant: Registry confirms update applied (Status=Updated, Capable=0x2). Remediation not needed."
                return $true
            }
            return $false
        }
        'InProgress' {
            # Stuck at 0x4104: bit 0x0004 (KEK deployment) not clearing after restarts.
            if (($availableInt -band 0x0004) -ne 0 -and -not $noError) {
                ...
                Write-Warning "  Treatment  : Treated as compliant - remediation cannot fix an OEM KEK signing gap."
                return $true
            }
            if (($availableInt -band 0x4100) -eq 0x4100) {
                Write-Host "Already compliant (Reboot pending): Update stage complete, awaiting reboot (Status=InProgress, AvailableUpdates=$availableStr). Remediation not needed."
            } else {
                Write-Host "Already compliant (In progress): Update is actively applying (Status=InProgress, AvailableUpdates=$availableStr). Remediation not needed."
            }
            return $true
        }
        'NotStarted' {
            if ($availableInt -ne 0 -and ($availableInt -band $AvailableUpdatesValue) -eq $AvailableUpdatesValue) {
                Write-Host "Already compliant (Pending task): Registry trigger already set (AvailableUpdates=$availableStr). Remediation not needed."
                return $true
            }
            return $false
        }
        'Failed' {
            Write-Warning "Cannot remediate (UEFICA2023Status=Failed): The update process encountered an error."
            Write-Warning "  Error Code : $errCode  |  Error Event : $errEvent"
            Write-Warning "  Action     : Review UEFICA2023Error and the System event log. An OEM firmware/BIOS update may be required."
            return $false
        }
...
try {
    ...
    if (-not $prereqOk) {
        ...
        # If the device became compliant after the detection script ran (e.g. Windows Update
        # applied the certificate between detection and remediation), skip remediation.
        Write-Host ''
        Write-Host '--- Compliance check ---'
        $isCompliant = Invoke-Detection -Diagnostics $diagnostics

        if ($isCompliant) {
            $finalStatus = 'Already compliant - remediation not needed'
            $exitCode    = 0
        } else {
            # ---- Remediation ----
            Write-Host ''
            ...
            if ($wasRemediated) {
                $finalStatus = if ($diagnostics['HWIsVM'] -eq 'Yes') {
                    ...
                    'Remediated - reboot may be required to complete certificate deployment'
                }
                $exitCode    = 0
            } else {
    ...
    Write-Host "  Was Compliant  : $isCompliant"
    Write-Host "  Was Remediated : $wasRemediated"
    Write-Host "  Exit Code      : $exitCode  (0 = Success, 1 = Failure)"
    ...
    try {
        ...
        Write-EventLog -LogName Application -Source $EventSource -EventId $evtId -EntryType $evtType `
            -Message "$ScriptName v$ScriptVersion`nStatus     : $finalStatus`nCompliant  : $isCompliant`nRemediated : $wasRemediated`nExit Code  : $exitCode"
